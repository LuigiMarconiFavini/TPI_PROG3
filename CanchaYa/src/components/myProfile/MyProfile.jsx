...
export default function MyProfile() {
  ...
  const updateMyProfile = async () => {
    try {
      if (formData.phone && !/^\+?\d{6,15}$/.test(formData.phone)) {
        setMsg("⚠️ Ingresá un teléfono válido (solo números y + opcional)");
        return;
      }
  ...
  return (
    <div
      className="min-h-screen bg-cover bg-center relative flex items-center justify-center px-4 py-4"
      style={{ backgroundImage: "url('/images/cancha-futbol.jpg')" }}
    >
      {/* Overlay oscuro/transparente */}
      <div className="absolute inset-0 bg-black/40"></div>

      {/* Card principal encima del overlay */}
      <div className="relative z-10 w-full max-w-lg p-8 rounded-xl shadow-xl bg-white/20 backdrop-blur-md border border-blue-500/40">
          ...
          <div>
            {editMode ? (
              ...
              <div className="space-y-4">
                ...
                {/* Teléfono */}
                <div className="p-3 text-base rounded-lg bg-white/10 backdrop-blur-lg border border-blue-400/20 shadow-sm hover:-translate-y-1 hover:scale-105 hover:shadow-md hover:border-blue-400/50 transition">
